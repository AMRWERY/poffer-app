...
// const checkedCategories = ref([]) // const tryChecked = ref([]) // async
function openFilterModal(){ // const modalFilter = await
modalController.create({ // component:
require('@/components/FilterModal.vue').default, // componentProps:
{"checkedCat":checkedCategories.value}, // initialBreakpoint: .90, //
breakpoints: [0, 0.9, .90, 0.75], // canDismiss: true, // handle: true, //
keyboardClose: true, // }); // modalFilter.onDidDismiss() // .then((data) => {
// console.log(data["data"], "data") // // const categories = data['data']; //
Here's your selected user! // // loadCityOffers(categories); // //
checkedCategories.value = categories; // if
(!tryChecked.value.includes(data["data"])){ //
tryChecked.value.push(data["data"]) // } // console.log(tryChecked.value) // });
